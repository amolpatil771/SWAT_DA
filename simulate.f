...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: idlst, j, iix, iiz, ic, mon, ii, dummy, jj,kk,iii
      real :: xx,d2,d3,d5,tempmean,tempvar1(6)
      real :: tempvar11(n_en_member)
      
      if (wrdtf == 1) then
      open  (525,file='smfc_1.dat')
      open  (526,file='solwp_1.dat')
      open  (527,file='solpor_1.dat')
      open  (528,file='smfc_2.dat')
      open  (529,file='solwp_2.dat')
      open  (530,file='solpor_2.dat')
      open  (534,file='solsat_1.dat')
      open  (535,file='solsat_2.dat')
      open  (538,file='sub_tc.dat')
      open  (605,file='wrt_1.dat')
      open  (606,file='wrt_2.dat')
      open  (607,file='smx.dat')
      write (525,1235) (sol_fc(1,kk), kk = 1,mhru)
      write (526,1235) (sol_wp(1,kk), kk = 1,mhru)
      write (527,1235) (sol_por(1,kk), kk = 1,mhru)
      write (528,1235) (sol_fc(2,kk), kk = 1,mhru)
      write (529,1235) (sol_wp(2,kk), kk = 1,mhru)
      write (530,1235) (sol_por(2,kk), kk = 1,mhru)
      write (534,1235) (sol_ul(1,kk), kk = 1,mhru)
      write (535,1235) (sol_ul(2,kk), kk = 1,mhru)
      write (538,1235) (sub_tc(kk), kk = 1,msub)
      write (605,1235) (wrt(1,kk), kk = 1,mhru)
      write (606,1235) (wrt(2,kk), kk = 1,mhru)
      write (607,1235) (smx(kk), kk = 1,mhru)
      
      !close (525,526,527,528,529,530,531,532,533,534,535,536)
      cn_flag(:,:) = 0
      end if
      da_daycount = 0
      if (da /= 0) then
      ...
      do curyr = 1, nbyr
       ...
       if (ifirstatmo == 1) then
        ...
        do i = id1, idlst                            !! begin daily loop
            da_daycount = da_daycount + 1
            !if (da_daycount == 731+503) then
            !  print *,da_daycount
            !endif
            !print *,da_daycount  
...
!!!%%% Update Surface Soil Moisure using Satelllite data and EnKF Data Assimilation %%%%%
      if (da == 1 .and. da_daycount <= da_stopday) then
              !print*, "Day_no", da_daycount
              
      do jj = 1,mhru
         d2 = sm_rfi(da_daycount,jj)
         d3 = sm_dqx(da_daycount,jj)
         d5 = smobs(da_daycount,jj)
        if (d5 > 0. .and. d2 < 0.4 .and. d3 < 0.1) then
        if (da_alg == 1) then
         call init_da(jj)
         call EnKF(jj)
         call post_da(jj)
        
        else if (da_alg == 2) then
         call init_da(jj)
         call SMAR_DA(jj)
         call post_da(jj)   
        end if
      ...
      if (wrdtf == 1) then
      do jj = 1,1
      open (543,file='correlation.dat')
      write (543,1423) ens_sol_st(1:n_en_member,1,jj),
     & ens_sol_st(1:n_en_member,2,jj)
      end do
      end if
      ...
      if (da_alg == 2) then
      do jj = 1,mhru
      if (da_flag(da_daycount,jj)==1.) then
      tmstp_all(da_daycount,jj) = 1.
      else if (da_daycount==1) then
      tmstp_all(da_daycount,jj) = 1.
      else
      tmstp_all(da_daycount,jj)=min(10.,tmstp_all(da_daycount-1,jj)+1.)
      end if
      end do
      
      do jj = 1,mhru
!      if (da_flag(da_daycount,jj)==1) then
      ens_sol_st_1(:,:,jj) = ens_sol_st(:,:,jj)
!      end if
      end do
      end if
      
1423  format(200(f11.5))

      if (wrdtf == 1) then
      open (539,file='inflow_flag.dat')          
      open (541,file='sm_std_obs.dat')
      open (542,file='precipday_all.dat')  
      open (544,file='surfq.dat')  
      open (545,file='tmstp_all.dat') 
      open (522,file='sm_1.dat')
      open (523,file='sm_2.dat')
      open (534,file='sm_sol_sw.dat')
      open (537,file='KG_ALL.dat')
      open (538,file='cn_flag.dat')
      open (540,file='da_flag.dat')
      open (601,file='cn.dat')
      open (602,file='sepbtm.dat')
      open (603,file='surq_final.dat')
      open (604,file='avg_temp.dat')
      open (546,file='sm_obs1.dat')
      open (547,file='sm_obs2.dat')
      write (539,1235)(inflow_flag(da_daycount,kk),kk=1,mhru)
      write (545,1235)(tmstp_all(da_daycount,kk),kk=1,mhru)
      write (601,1235)(cnday(kk), kk = 1,mhru) 
      write (522,1235) (sol_st(1,kk), kk = 1,mhru)
      write (523,1235) (sol_st(2,kk), kk = 1,mhru)
      write (534,1235) (sol_sw(kk), kk = 1,mhru)
      write (537,1235) (KG_all(da_daycount,kk), kk = 1,mhru)
      write (538,1235) (cn_flag(da_daycount,kk), kk = 1,mhru)
      write (540,1235) (da_flag(da_daycount,kk), kk = 1,mhru)
      write (541,1235) (sm_std_obs_all(da_daycount,kk),kk = 1,mhru)
      write (542,1235) (precipday_all(da_daycount,kk),kk = 1,mhru)
      write (544,1235) (surfq(kk), kk = 1,mhru) 
      write (602,1235) (sepbtm(kk),kk = 1,mhru)
      write (603,1235) (surfq(kk),kk = 1,mhru)
      write (604,1235) (tmpav(kk),kk = 1,mhru)
      write (546,1235) (sm_obs1(da_daycount,kk),kk = 1,mhru)
      write (547,1235) (sm_obs2(da_daycount,kk),kk = 1,mhru)
      end if
 
1235  format(2x, *(f10.4))
...
!          if (biomix(j) > .001) call tillmix (j,biomix(j))
     ...
     &                     mat_yrs(idplt(j)))
      ...
      return
 1234 format (1x,' Executing year ', i4,i2)
      end