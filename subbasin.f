...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j,sb,kk
      real :: tmpk, d, gma, ho, pet_alpha, aphu, phuop,d1
...
!     &        then             
       ...
       !!   surfq_ru(j) = 0.
        ...
        inflpcp = Max(0.,precipday - surfq(j))
          ...
          ! compute ensemble of effective rainfall 
        do ii = 1,n_en_member
        ens_inflpcp(ii,1)=(ens_precip(ii,1)-ens_surfq(ii,j))
        end do
        inflpcp = sum(ens_inflpcp(1:n_en_member,1))/n_en_member
        if (inflpcp<0.) then
            ens_inflpcp(1:n_en_member,1) = 0.
           inflpcp = 0.
        end if
  
        end if
        inflow_flag(da_daycount,j) = inflpcp
...
!        if (pot_vol(j) < 1.e-6) call etact
       ...
       !! computele depth using climate drivers
        call wattable
