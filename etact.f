...
!!    real, parameter :: esd = 500., etco = 0.80, effnup = 0.1
      ...
      real :: sumsnoeb, evzp, eosl, dep, evz, sev, dummy1,sol_st_temp
      !daily evaporation for each layer
      real :: et11, et12, et13
      real :: sstg1, sstg2, sstg11, sstg22
...
!! take soil evap from each soil layer
     ...
     &       .00713 * sol_z(ly,j)))
          ...
          !dummy1 = 5*Expo(-(sol_z(ly,j)/1400))
          !xx =   dummy1 * (sol_st(ly,j) - sol_fc(ly,j))/ sol_fc(ly,j) 
          !if (ly == 1) then
          !    xx =  6 * (sol_st(ly,j) - sol_fc(ly,j)) / sol_fc(ly,j)
          !else if (ly == 2) then
          !    xx =  4 * (sol_st(ly,j) - sol_fc(ly,j))/ sol_fc(ly,j)
          !else 
          ...
          else      ! DA calculations
              if (sol_st(ly,j) > sev) then
              esleft = esleft - sev
            sol_st(ly,j) = Max(1.e-6, sol_st(ly,j) - sev)
              do ii = 1,n_en_member
                ens_sol_st(ii,ly,j)=max(1.e-6,(ens_sol_st(ii,ly,j)-sev))
              end do
          sol_st_temp=sum(ens_sol_st(1:n_en_member,ly,j))/n_en_member
          if (sol_st_temp < 0) then
                ens_sol_st(1:n_en_member,ly,j) = 0.
                sol_st(ly,j) = 0.
          elseif ((sol_st_temp-sol_st(ly,j))>1.e-4) then
          ens_sol_st(1:n_en_member,ly,j)=ens_sol_st(1:n_en_member,ly,j)*
     &       (sol_st(ly,j)/sol_st_temp)
          endif
      ...
      do ly = 1, sol_nly(j)
      do ii = 1,n_en_member
          ens_sol_sw(ii,j)=ens_sol_sw(ii,j)+ens_sol_st(ii,ly,j)
      end do
      end do
      ...
      if (j > 0 .and. j < 19 .and. wrdtf == 1) then
      open (571,file='evap.txt')
      write (571,1234) pet_day,es_max,es_day,et11,et12
1234  format(3x, 6(f11.5))
      end if
