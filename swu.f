...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, k, ir, ii,ly
      real, dimension(mlyr) :: wuse
      real :: sum1, xx, gx, reduc, sump
...
!         endif
         ...
         do ii = 1,n_en_member
         ens_sol_st(ii,ly,j) = (ens_sol_st(ii,ly,j) - wuse(k))
         end do
        ...
        if (da==0) then
          ...
          do k = 1, sol_nly(j)
          do ii = 1,n_en_member 
            ens_sol_sw(ii,j)=ens_sol_sw(ii,j)+ens_sol_st(ii,k,j)
          end do
