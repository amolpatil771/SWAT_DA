...
!! Variables for soil P and additional operations mjw
	...
	allocate(rspc_a(mhru))	
      ...
      !!By Amol for Data Assimilation
      allocate (ens_sepday(n_en_member,1))
      allocate (ens_sw_excess(n_en_member,1))
      allocate (ens_qlyr(n_en_member,1))
      allocate (ens_ul_excess(n_en_member,1))
      allocate (ens_sol_sw(n_en_member,mhru))
      allocate (ens_surfq(n_en_member,mhru))
      allocate (ens_inflpcp(n_en_member,1))
      allocate (ens_latlyr(n_en_member,1))
      allocate (ens_ho(n_en_member,1))
      allocate (sm1(mhru))
      allocate (ens_sol_st(n_en_member,mlyr,mhru))
      allocate (ens_sol_st_1(n_en_member,mlyr,mhru))
      allocate (ens_sol_fc(n_en_member,mlyr,mhru))
      allocate (smobs(n_days_sim,mhru))
      allocate (pcp_stn(n_days_sim,mhru))
      allocate (sm_dqx(n_days_sim,mhru))
      allocate (sm_rfi(n_days_sim,mhru))
      allocate (inflow_flag(n_days_sim,mhru))
      allocate (ens_sev(n_en_member,1))
      allocate (ens_esleft(n_en_member,1))
      allocate (da_flag(n_days_sim,mhru))
      allocate (sm_std_obs_all(n_days_sim,mhru))
      allocate (precipday_all(n_days_sim,mhru))
      allocate (cn_flag(n_days_sim,mhru))
      allocate (KG_all(n_days_sim,mhru))
      !For EnKF
      allocate (xbb(no_sta_var,n_en_member))
      allocate (xbb_1(no_sta_var,n_en_member))
      allocate (xaa(no_sta_var,n_en_member))
      allocate (y(no_obs_var,n_en_member))
      allocate (err_xb(no_sta_var,n_en_member))
      allocate (err_y(no_obs_var,n_en_member))
      allocate (st_er_cov(no_sta_var,no_sta_var))
      allocate (st_par_cov(no_sta_var,1))
      allocate (obs_er_cov(no_obs_var,no_obs_var))
      allocate (st_obs_cov(no_sta_var,no_obs_var))
      allocate (kalman_gain(no_sta_var,no_obs_var))
      allocate (peakr_write(mhru))
      allocate (sm_max(msub,3))
      allocate (a_smar(msub,2))
      allocate (b_smar(msub,2))
      allocate (sw2_smar(msub,2))
      allocate (sc1_smar(msub,2))
      allocate (tmstp_all(n_days_sim,mhru))
      allocate (sm_obs1(n_days_sim,mhru))
      allocate (sm_obs2(n_days_sim,mhru))
