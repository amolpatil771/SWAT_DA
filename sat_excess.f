...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, j1, ii, isp, iii
      real:: ul_excess,qlyr,pormm
...
!         end if
 	...
 	if (isep_opt(j)==2.and.j1==i_sep(j)) then
	  ...
	  ens_qlyr(1:n_en_member,1) = ens_sol_st(1:n_en_member,ii,j)
        
        qlyr = sum(ens_qlyr(1:n_en_member,1))/n_en_member
	  ...
	  do while (qlyr>0.and.ii>1)
	   ...
	   if (sol_st(ii,j) > sol_ul(ii,j)) then
	     ...
	     !qlyr = max(sol_st(ii,j) - sol_ul(ii,j),0.) 	! excess water moving to upper layer
	     !sol_st(ii,j) = sol_st(ii,j) - qlyr  
           do iii=1,n_en_member
           ens_qlyr(iii,1)=(ens_sol_st(iii,ii,j) - sol_ul(ii,j))
           end do
           qlyr = sum(ens_qlyr(1:n_en_member,1))/n_en_member
           if (qlyr<0.) then
               ens_qlyr = 0.
           ...
           do iii=1,n_en_member
           ens_sol_st(iii,ii,j) = ens_sol_st(iii,ii,j) - ens_qlyr(iii,1)
           end do
           sol_st(ii,j) = sum(ens_sol_st(1:n_en_member,ii,j))/
     &         n_en_member
     ...
     &                   + sptno2concs(isp))  
	     ...
	     ! add soil moisture and nutrient to upper layer
         do iii=1,n_en_member
             ens_sol_st(iii,ii-1,j) = ens_sol_st(iii,ii-1,j)+ 
     &        ens_qlyr(iii,1) 
         end do
         sol_st(ii-1,j) = sum(ens_sol_st(1:n_en_member,ii-1,j))/
     &    n_en_member
	     !sol_st(ii-1,j) = sol_st(ii-1,j) + qlyr ! add excess water to upper layer
	     sol_no3(ii-1,j) = sol_no3(ii-1,j) + xx * (sptno3concs(isp) 
     &                   + sptno2concs(isp))  
	   ...
	   if (ii==2) then
	     !qlyr = sol_st(1,j) - sol_ul(1,j)
           do iii=1,n_en_member
           ens_qlyr(iii,1) = ens_sol_st(iii,1,j) - sol_ul(1,j)
           end do
           qlyr = sum(ens_qlyr)/n_en_member
           
	     ! excess water makes surface runoff
	     if (qlyr>0) then
	         ens_sol_st(1:n_en_member,1,j) = sol_ul(1,j)
               sol_st(1,j) = sum(ens_sol_st(1:n_en_member,1,j))/
     &               n_en_member
	         !surfq(j) = surfq(j) + qlyr 
           do iii = 1,n_en_member
           ens_surfq(iii,j) = ens_surfq(iii,j)+ens_qlyr(iii,1)
           end do
     ...
     &                      * (sptno3concs(isp) + sptno2concs(isp)) 
      ...
      if (sol_st(j1,j) - sol_ul(j1,j) > 1.e-4) then
          ...
          !sol_st(j1,j) = sol_ul(j1,j)
      do iii=1,n_en_member
	ens_sepday(iii,1)=ens_sepday(iii,1)+(ens_sol_st(iii,j1,j)-sol_ul(j1,j))
      end do 
      sepday=sum(ens_sepday)/n_en_member
      
      do iii=1,n_en_member
      ens_sol_st(iii,j1,j)=sol_ul(j1,j)
      end do 
      sol_st(j1,j) = sum(ens_sol_st(1:n_en_member,j1,j))/n_en_member
        ...
        if (sol_st(j1,j) - sol_ul(j1,j) > 1.e-4) then
          ...
          !sol_st(j1,j) = sol_ul(j1,j)
        do iii=1,n_en_member
	  ens_ul_excess(iii,1) = ens_sol_st(iii,j1,j)-sol_ul(j1,j)
        end do
        ul_excess=sum(ens_ul_excess)/n_en_member
        do iii=1,n_en_member
        ens_sol_st(iii,j1,j)=sol_ul(j1,j)
        end do
        sol_st(j1,j) = sum(ens_sol_st(1:n_en_member,j1,j))/n_en_member
      ...
      do ly = nn - 1, 1, -1
            do iii=1,n_en_member
                ens_sol_st(iii,ly,j)=ens_sol_st(iii,ly,j)+
     &           ens_ul_excess(iii,1)
            end do 
         sol_st(ly,j) = sum(ens_sol_st(1:n_en_member,ly,j))/n_en_member
         ...
         if (sol_st(ly,j) > sol_ul(ly,j)) then
           ...
           !sol_st(ly,j) = sol_ul(ly,j)
          do iii=1,n_en_member
           ens_ul_excess(iii,1) = ens_sol_st(iii,ly,j) - sol_ul(ly,j)
          end do
         ul_excess=sum(ens_ul_excess)/n_en_member
         do iii=1,n_en_member
         ens_sol_st(iii,ly,j) = sol_ul(ly,j)
         end do
         sol_st(ly,j) = sum(ens_sol_st(1:n_en_member,ly,j))/n_en_member
          else
              ul_excess = 0.
