...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, j1, nn, k, sb,isp,ii, dummy
      real :: swex_var,swex_std,fc_std
      real :: rn_da_fc(n_en_member,1)
      real, dimension(:), allocatable :: fc_std11,sol_std11
      
      j = ihru
      allocate (fc_std11(sol_nly(j)))
      allocate (sol_std11(sol_nly(j)))
...
!       Updated water table depth D.Moriasi 4/8/2014
      ...
      if (icrk == 1) then
        do ii=1,n_en_member
        ens_sepday(ii,1) = ens_inflpcp(ii,1) - voltot
        end do
        sepday = sum(ens_sepday)/n_en_member
        if (sepday<0) then
            ens_sepday(1:n_en_member,1)=0.
            sepday=0.
        end if
        
      else
        do ii=1,n_en_member
          ens_sepday(ii,1) = ens_inflpcp(ii,1)
        end do 
        sepday = sum(ens_sepday)/n_en_member
      end if
	...
	if (aird(j)>0) then
      ...
      do ii=1,n_en_member
      ens_sepday(ii,1) = ens_inflpcp(ii,1)+ aird(j) + pot_seep(j)
      end do
      sepday = sum(ens_sepday)/n_en_member
      pot_seep(j) = 0.
      ...
      do j1 = 1, sol_nly(j)
          ...
          do ii=1,n_en_member
          ens_sol_st(ii,j1,j) = ens_sol_st(ii,j1,j) + ens_sepday(ii,1)
          end do
          
          sol_st(j1,j) = sum(ens_sol_st(1:n_en_member,j1,j))/n_en_member
	  ...
	  if(j1==i_sep(j).and.sol_tmp(j1,j) > 0. .and. isep_opt(j) /= 0) then
          ...
          do ii=1,n_en_member
          ens_sol_st(ii,j1,j) = ens_sol_st(ii,j1,j)+ qstemm(j)
          end do
          sol_st(j1,j) = sum(ens_sol_st(1:n_en_member,j1,j))/n_en_member
     ...
     &                    + sptno2concs(isp))  
       ...
       !! determine gravity drained water in layer
        ...
        !sw_excess = sol_st(j1,j) - sol_fc(j1,j)
!        if ((sol_st(j1,j) - sol_fc(j1,j)) > -3.00) then
        
        swex_var = 0 
        swex_std = 0
       do kk=1, n_en_member
          swex_var=swex_var+(((ens_sol_st(kk,j1,j)-sol_st(j1,j))**2.0)
     &      /n_en_member)
       end do
       if (swex_var>0.) then
        swex_std = sqrt(swex_var)    
       else
           swex_std = 0.
       end if
...
!!! generate randomnumber with 1 correlation to sol_st ensemble            
        if (swex_var>0.) then    
        rn_da_fc(1:n_en_member,1) = ens_sol_st(1:n_en_member,j1,j)
     &   -sol_st(j1,j)
        rn_da_fc(1:n_en_member,1) = rn_da_fc(1:n_en_member,1)/swex_std
        else
           rn_da_fc(1:n_en_member,1) = 0.
        end if
        
        if (j1==1) then
          fc_std = sol_fc(j1,j)*fcpercent  
        else if (j1==2) then
          fc_std = sol_fc(j1,j)*fcpercent2    
        else if (j1==3) then
          fc_std = sol_fc(j1,j)*fcpercent3  
        end if
        
        
                                                                  !fc_std=min(sol_fc(j1,j)*fcpercent,(fc_std11(j1) + sol_std11(j1)))
        do ii = 1,n_en_member
         ens_sol_fc(ii,j1,j)=((sol_fc(j1,j))+
     &    ((rn_da_fc(ii,1)*fc_std)))
        end do

        do ii=1,n_en_member
          ens_sw_excess(ii,1)=ens_sol_st(ii,j1,j)- ens_sol_fc(ii,j1,j)
        end do
        sw_excess=sum(ens_sw_excess)/n_en_member
        
        !else
        !do ii=1,n_en_member
        !  ens_sw_excess(ii,1)=ens_sol_st(ii,j1,j)- sol_fc(j1,j)
        !end do
        !sw_excess=sum(ens_sw_excess)/n_en_member
        !end if
        ...
        if (sw_excess > 1.e-5) then
          ...
          !! percolation (sepday)
          call percmicro(j1)       ! Calculates latlyr and seepage to next layer
          
          do ii=1,n_en_member
              ens_sol_st(ii,j1,j) = ens_sol_st(ii,j1,j) - 
     &         ens_sepday(ii,1) - ens_latlyr(ii,1) - lyrtile
          end do
!          ens_sol_st(1:n_en_member,j1,j) = 
!     &     Max(1.e-6,ens_sol_st(1:n_en_member,j1,j))
          
          sol_st(j1,j) = sum(ens_sol_st(1:n_en_member,j1,j))/n_en_member
          if (sol_st(j1,j) < 1.e-6) then
              ens_sol_st(1:n_en_member,j1,j) = 0.
              sol_st(j1,j) = 0.
          end if
          !sol_st(j1,j) = sol_st(j1,j) - sepday - latlyr - lyrtile
          !sol_st(j1,j) = Max(1.e-6,sol_st(j1,j))
...
!         sol_st(j1,j) = Max(1.e-6,sol_st(j1,j))
      ...
      !! update soil profile water
      sol_sw(j) = 0.
      ens_sol_sw(1:n_en_member,j) = 0.
      do j1 = 1, sol_nly(j)
        do ii = 1,n_en_member
          ens_sol_sw(ii,j) = ens_sol_sw(ii,j)+ ens_sol_st(ii,j1,j)
        end do
      end do
      sol_sw(j)=sum(ens_sol_sw(1:n_en_member,j))/n_en_member
      ...
      if (qtile > 0.) then
        ...
        do j1 = 1, sol_nly(j)
          ...
          if (xx > 0.) then
            ...
            !sol_st(j1,j) = sol_st(j1,j) - sumqtile
                do ii = 1,n_en_member
                ens_sol_st(ii,j1,j)=ens_sol_st(ii,j1,j)-sumqtile
                end do
           sol_st(j1,j)=sum(ens_sol_st(1:n_en_member,j1,j))/n_en_member
              sumqtile = 0.
            else
           ...
           ens_sol_st(1:n_en_member,j1,j) = sol_fc(j1,j)
