...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j,ii
      real :: adjf, yy, dg, ho, ratio, sol_k_sep, const
...
!      !! DA loop ----------------%%%%%%%%%%%%%%%%%%%
      ...
      if (sol_tmp(ly1,j) <= 0.) then
        ...
        do ii=1,n_en_member
        ens_ho(ii,1) = 2. * ens_sw_excess(ii,1) / 
     &                ((sol_ul(ly1,j) - sol_fc(ly1,j)) /  dg)
        end do
        ho = sum(ens_ho(1:n_en_member,1)/n_en_member)
        end if
        do ii=1,n_en_member
        ens_latlyr(ii,1)=adjf * ens_ho(ii,1)*sol_k(ly1,j) * hru_slp(j) /
     &               slsoil(j) * .024
        end do
        latlyr = sum(ens_latlyr)/n_en_member
        if (latlyr < 0.) latlyr = 0. 
        if (latlyr < 0.) ens_latlyr = 0.
        if (latlyr > sw_excess) latlyr = sw_excess
        if (latlyr > sw_excess) ens_latlyr = ens_sw_excess
          latlyr = sum(ens_latlyr)/n_en_member
      sol_hk(ly1,j) = (sol_ul(ly1,j) - sol_fc(ly1,j)) / sol_k(ly1,j)
...
!!  septic changes 1/28/09
      ...
      ens_sepday = 0.
      do ii=1,n_en_member
          ens_sepday(ii,1) = ens_sw_excess(ii,1) * 
     &     (1. - Exp(-24. / sol_hk(ly1,j)))
      end do
	...
	if(ly1 == i_sep(j).and.isep_opt(j)==1) then
	   do ii=1,n_en_member
          ens_sepday(ii,1) = min(ens_sepday(ii,1),sol_k_sep *24.)
         end do
         sepday = sum(ens_sepday)/n_en_member
	   bz_perc(j) = sepday
	end if
      ...
      if (ly1 == sol_nly(j)) then
        ...
        if (xx < 1.e-4) then
          ...
          !sepday = sepday * xx / (xx + Exp(8.833 - 2.598 * xx))
        const = xx / (xx + Exp(8.833 - 2.598 * xx))
          
        ens_sepday(1:n_en_member,1)=ens_sepday(1:n_en_member,1) * const
          
        sepday = sum(ens_sepday(1:n_en_member,1))/n_en_member
        end if
      end if
      ...
      ens_sepday(1:n_en_member,1)=ens_sw_excess(1:n_en_member,1)*ratio
      sepday = sum(ens_sepday(1:n_en_member,1))/n_en_member
        
        ens_latlyr(1:n_en_member,1)=ens_sw_excess(1:n_en_member,1)*
     &   (1. - ratio)
        latlyr = sum(ens_latlyr(1:n_en_member,1))/n_en_member
       endif
       if (sepday + lyrtile > sw_excess) then
        sepday = 0.
        ens_sepday = 0.
        !sepday = sw_excess - lyrtile
          do ii=1,n_en_member
          ens_sepday(ii,1)=ens_sw_excess(ii,1) - lyrtile
          end do
          sepday = sum(ens_sepday)/n_en_member
      endif  
