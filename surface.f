...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &       * (1.- fimp(urblu(j))) * 1000.) !mm/dt
      ...
      !!  read random number in ensemble array
      if (da == 1) then
          rand_da (1:n_en_member,1) = ens_samp(cnt_strt:cnt_end,1)
      end if
      ...
      !! DA ensemble 
      ens_surfq(1:n_en_member,j)=ens_surfq(1:n_en_member,j)+qird(j)
